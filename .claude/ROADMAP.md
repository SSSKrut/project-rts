...
> **Пивот 2026-08-25 — этот файл больше не трекер, а АРХИВ фаз 0-20.**
> История решений ценна и на неё ссылаются, но порядок дальнейших фаз отменён.
> Что делаем дальше — `.claude/lite/PLAN.md`.
...
22. **Размер карты — свободный параметр, а не ограничение движка** (зафиксировано 2026-08-23 при разборе Phase 20). У мира нет границы: стриминг чанков вокруг якоря, `WorldPos{Chunk, Local}` с int32-координатами чанка, манифест `maps/<name>.json` задаёт сид и содержимое, но не экстент. Единственное, что сегодня «размером с карту» — прожжённый хиллшейд 2D-панели: `BakeUnderlay(0, 0, 2000, 4)`, квадрат 2×2 км при 4 м/пиксель; дальше панель рисует фон, а симуляция идёт как обычно. Расширение = два числа и размен памяти на разрешение (10 км при 4 м/пикс ≈ 25 МБ RGBA, при 8 м/пикс ≈ 6 МБ); ни нав, ни стриминг, ни координаты не трогаются. Следствие: всё, что упирается в «карта маловата» — бомбардировщики (`bomber_*` запечены и отложены до стратегического слоя), дальнобойная артиллерия, оперативные расстояния — разблокируется решением о масштабе операции, а не работой по движку.
23. **Multi-file systems**: когда система переваливает за ~700 строк И есть ≥2 независимые фазы / domain slice, разносим в `XXX.go` orchestrator + sibling файлы `XXX_phase.go` в том же пакете. Go биндит методы поверх файлов; публичный API не меняется. Применено к `spatial_bake` (4 passes), `weapon` (snapshot/resolve/postpass), `unit_movement` (step/walls), `order_resolver` (completion/target), `survival_instinct` (scramble/cover), `squad_service` (membership/template/orders), `ui/inspector` (unit/squad/orders/override). Не разносим монолитную функцию по двум файлам — это хуже LOC-цены.
...
### Phase 19.8 — Движение 2.1: интерьеры, слоты, уступание дороги 🚧 (M1-M5 по коду 2026-08-09; остался owner-плейтест)

**Цель.** Закрыть owner-фидбек «плохо заходят в здания, встают на этажах, не подходят к окнам». Аудит 2026-08-09 (весь стек движения + 17 сцен PASS) свёл симптомы к четырём корням + двум сквозным дефектам нижнего слоя: (1) интерьерные слоты раздаются по индексу ростера, а не по близости — потоки крестятся, KIA сдвигает всю раскладку; (2) комнатные слоты не валидируются геометрией (Loose-офсет 1.2–2.4 м без клэмпа в AABB комнаты → слот в стене/на лестнице); (3) стоящая пехота не уступает дорогу пехоте — боец в проёме блокирует этаж навсегда (blocked-pop не трогает гейты, реплан слеп к телам); (4) combat-throttle ×0.3 + prone душат вход по приказу; (5) позиционные толчки (шов техники/юнитов, сайдстеп, contact clamp) двигают позицию мимо стен — idle-юнита продавливает сквозь фасад; (6) фантомная скорость после остановки (>2 с призрачного VelX/VelZ в хеше — ORCA дёргается от призраков при рассадке).

- M1 ✅: стеночный клип для ВСЕХ позиционных мутаций (шов техники / коридорный сайдстеп / расталкивание / contact clamp) — шаг доходит до фасада и остаток скользит вдоль него; watchdog мёртвой зоны у цели за стеной. Обнуление скорости на остановке отменено по замеру (ломает когезию колонны).
- M2 ✅: назначение слотов по стоимости доставки со СТРУКТУРНЫМ удержанием (`LocalBlackboard.HeldSlot`, index+1 чтобы нулевое значение читалось как «нет»); при нехватке людей отбрасывается хвост списка, а не дальние комнаты; окна без фасинга — round-robin по фасадам.
- M3 ✅: клэмп слота в AABB комнаты с отступом 0.6 + выталкивание из транзитных кругов (дверные проёмы, оба конца лестничного пролёта).
- M4 ✅: make-way — стоящий уступает тому, кто РЕАЛЬНО идёт мимо (давление ≥0.3 с от движущегося тела), шаг клипается о стены; возврат через formation re-push.
- M5 ✅: интерьерный приказ снимает combat-throttle ×0.3 (но НЕ наводку на угрозу — она и есть прицеливание). Главный выигрыш фазы: `ai_clear_building` 40.8 → 14.9 с.

**Юнит-тесты как часть фазы:** каждая чистая функция слоя движения покрыта таблично (8 новых `_test.go` в `systems/`). Тесты нашли баг знака в ORCA (правая нога конуса скоростей: RVO2 отрицает вектор целиком, здесь — только X, полуплоскость зеркалится) и противоречие в док-комментарии `orcaLine`. Комментарий исправлен; сам фикс ОТКАЧЕН вместе с retune-пассом — он преображает поток (`ai_crowd_cross` 22.4 → 8.7 с, рывок 5.2 → 1.2), но выбивает отряд в `ai_bounding`. Регрессионный тест помечен `t.Skip` с причиной.

**Второй проход тестами** (нав / рельеф / баллистика / RoadRouter / cover-слоты, ещё 7 файлов): эвристика A* была неадмиссибельна при `RoadPrefer` и `CoverSeek` (эвристика 2.0/м против реального метра в 1.0 — поиск жадный и игнорирует ВЫБРАННЫЙ стиль); `maskProtection` голосовала всей полусферой по cos² вместо двух охватывающих секций, из-за чего стена, ровно перекрывающая азимут, давала максимум 0.5 при 1.0 у соседней ветки — маскированные укрытия систематически проигрывали ранжирование; `components.Normalize` могла вернуть `Local == ChunkSize` (float32 округляет `-1e-7 + 64` до 64.0); ключ дедупликации угла занимался до проверки биссектрисы, и коллинеарная пара стен сжигала вершину для третьей стены. Все четыре исправлены. Запрет среза угла по диагонали в A* попробован и отвергнут по замеру — дверная клетка сама footprint-клетка, легализованная исключением, и требование открытого соседа закрывает единственный вход.

Не в фазе: очередь входа для Garrison, slope-churn марша, интерьерные позиции SurvivalInstinct, LOS сквозь перекрытия, зазор внешних слотов до фасада (`clampSlotXZ`). Отклонения от плана с замерами — `PHASE-19.8.md`.

**Зависимости.** Phase 19.5 (SquadBrain/ClearSeq, blocked-pop, string-pull), Phase 17.9 (multi-storey nav), WS-B (гейты; baseline пересъёмка в конце — фаза поведенческая).

### Phase 20 — Авиация: вертолёты и ПВО 🚧 M0-M2 закрыты, играбельный срез достигнут (план — `PHASE-20-AIR.md`)

**Цель.** Вертолёты (ударный + транспортный) как отдельный locomotion class **вместе с ПВО** — без зенитных средств фаза непроверяема. Ядро — не полёт, а **управление излучением**: активный канал видит дальше и выдаёт себя, пассивный слеп и незаметен; решение принимают обе стороны, и именно оно делает ПВО объектом, который можно переиграть, а не статической опасностью.

Разбор 2026-08-23 сузил рамку прежней «Aviation + interop»: фронтовая авиация и заявки на вылет вынесены в отдельную мини-фазу, наземный interop — в Phase 20.5.

- Управление приборное, не рулевое (модель Sea Power): маршрут по существующей ПКМ-грамматике + **липкие дилы** — приборные скорость и высота, состояние излучающих средств, RoE. Никакого click-spam'а.
- Высота непрерывна; полосы (NOE / низкая / средняя / транзит) — производный классификатор для трёх вопросов: кто видит, кто достаёт, маскирует ли рельеф. `AltRef` = AGL | AMSL.
- Обнаружение воздуха. M1 сделал борт кандидатом и наблюдателем в общем проходе `ContactSystem`, а не отдельным aircraft-major проходом (второй набор предикатов LOS/стен/рельефа разошёлся бы с первым; бортов единицы, экономия мнимая). Маскировка «только на бреющем» вышла даром — `terrainBlocksLOS` уже height-aware. Сигнатура зависит от полосы высоты в обе стороны: на бреющем тише для глаза и громче для уха.
- ПВО: зенитный ствол со стрельбой с упреждением (`aa_gun_*` уже запечены) + ПЗРК как **сущность-ракета** (только у летящей ракеты есть время на реакцию, только так ловушки — решение). Розыгрыш — детерминированный хеш, не `math/rand`.
- `AircraftOverride{Break, Flare, Abort}` — зеркало `VehicleOverride` из 19 M4, без приватного cross-tick состояния.
- Излучение (M1, ядро фазы): `SensorChannel.EmitRangeM` (>0 = активный излучатель, слышен дальше, чем видит) + `Sensors.OffMask` (выключено, не включено — нулевое значение обязано означать «всё работает»). Канал `SensorESM` даёт **пеленг без дальности** (`Contact.BearingOnly`); второй источник превращает направление в точку. Пассивный контакт НЕ пишет в `Awareness` — там ждут позицию, а у пеленга её нет, и запись сделала бы ESM строго лучше глаз.
- `FireOnAir` (лежит без читателя с Phase 13) + множитель `VsAir` в `WeaponSpec` — поле заведено в M1 и равно нулю у всего роста, и **ноль ЕСТЬ гейт**: без него борт в `Awareness` читается как мягкая цель и техника жжёт ленту по вертолёту, которого нет в пуле целей. M2 заполняет цифрами.
- Борт не входит ни в `NavGrid`, ни в ORCA, ни в `VehicleSpatialHash`, ни в `RoadSurface`, ни в `GroundStick`.
- Суб-шаг по скорости вместо переменного dt (переменный шаг ломает детерминизм и save/load).
- Борт приходит **по расписанию сценария** у края карты и уходит за край; топливо и боекомплект настоящие, но их исчерпание = уход, а не посадка. Площадка / FARP — второй вариант поверх той же машинерии: расписание и площадка обязаны быть двумя источниками одного события «борт доступен», иначе вариант 2 придётся переписывать. Абстрактный пул вылетов — вместе с фронтовой авиацией. Загрузка выбирается при постановке задачи, до появления борта.
- Воздушная обстановка — символикой **поверх общей карты**; отдельного вида не заводим (фильтр — аварийный выход, если карта окажется перегружена на длинной операции).
- **Joint orders**: `OrderJoint{Participants, BarrierKind}` + `BarrierBoard` / `BarrierDismount` — минимум, нужный высадке с вертолёта; 20.5 переиспользует ту же механику для БТР. См. `COMMAND-MODEL.md` §3.
- Воздушные события в `EventLog` + `AttentionMatrix` (под 8× пролёт длится две секунды настенного времени).

**Майлстоуны.** M0 полёт ✅ → M1 обнаружение и излучение ✅ → M2 ПВО ✅ (2026-08-24: `VehicleAAGun` — эмплейсмент как класс техники со скоростями 0; воздушный выстрел — аналитическая ветка вместо рейкаста, у которого свой тихий потолок 192 м и наземный пул целей; ПЗРК `WeaponIgla` спавнит `Missile`-сущность, ловушки — ОДИН детерминированный розыгрыш в терминальном окне; `AircraftOverride` Break/Flare/Abort зеркалит `VehicleOverride`, Abort = `SendHome`; `VehicleReflexGoDark` глушит радар расчёта под огнём без ответа; бейк `aa_gun_*` разложен на hull+turret конфигами по именам генератора; сцены `ai_air_aa_gun` + `ai_air_manpads`; попутно: уход за край не выметал Awareness — латентно с M0, SIM PANIC в utility) → **M3 удар с воздуха** → M4 транспорт и barrier'ы → M5 карта и внимание → M6 гигиена. Порядок кодирует правило «ПВО раньше удара»: играбельный срез — после M2, разведка с дилеммой излучения против ПВО, которая сама решает, светиться ли.

**Зависимости.** Phase 19 (спек-таблицы, фабрики, кинематический драйвер, рефлексы), Phase 18.5 (Sensors / Contact / FoW), Phase 19.7 (слой внимания, карта как командная поверхность), Phase 17.5-MODELS (модели `heli_*` / `gunship_*` / `aa_gun_*` запечены).

### Phase 20.5 — Infantry-Vehicle interop 📋

**Цель.** Режимы совместного движения пехоты и наземной техники. Отделено от 20 в разборе 2026-08-23: общего с авиацией — только barrier-механика совместных приказов, которая пишется в 20 M4 и здесь переиспользуется без правок.

- `EmbarkedIn{Vehicle, Mode}` компонент на Squad'е (Mounted / Riding).
- `EscortPair{With, Mode}` компонент (Following / Escorting).
- Order types: `Mount`, `Disembark`, `Follow`, `Escort` — поверх `OrderJoint` из Phase 20.
...
**Зависимости.** Phase 20 M4 (barrier-механика), Phase 19 (техника), Phase 11 (Order linked-list infrastructure).
...
### Phase 20.7 — Прозрачность командной поверхности 🚧 L0-L1 закрыты (план — `PHASE-20.7-LEGIBILITY.md`)

**Цель.** Закрыть пять вещей, в которые уперся плейтест Phase 20 M1 (2026-08-24): не видно, куда летит борт; не видно дистанций сенсоров; не подписаны приборные элементы управления (`AGL`, `Cruise`); в таймлайне нет техники; не видно, докуда юнит обнаруживает противника, и как следствие нечем отдать огонь на подавление без контакта.

Это три задачи, а не пять, и корень у первой — не в интерфейсе.

- **Одиночник не имеет приказа.** Каждая поверхность «что этот юнит делает» — метки на карте, строки таймлайна, блок приказов инспектора — построена на сущностях `Order` и итерируется фильтром по `Squad`. Одиночная техника и борт получают `Action` прямо в `ActionQueue` через `pushSoloMove`, минуя `SquadService`. У вертолёта нет отметки цели не потому, что её забыли нарисовать, а потому, что рисовать нечего.
- **Решено делать глубоко** (владелец, 2026-08-24), но НЕ через «отряд из одного»: буквальный отряд втащил бы формацию и мозг отряда на грузовик. Шов проходит там, где маркер `Squad` сегодня значит сразу две несвязанные вещи. Разведено: **`OrderQueueHead` = «этим можно командовать независимо»** (плюс `CommandRoster` из себя, чтобы шесть мест в резолвере ответили сами), **`Squad` = отрядное ПОВЕДЕНИЕ**. Все отрядные системы уже фильтровались по маркеру — шов был в коде, им просто не пользовались, поэтому глубокая правка стоила почти ничего вместо обещанного переписывания. Одиночник получает историю, прогресс, цепочку и отмену наравне с отрядом.
- **Охват — постоянный индикатор выделения**, из собственной позиции, на карте и в 3D. Hold-V остаётся тем, чем является: гипотетической точкой для планирования. И у охвата ДВА радиуса: «докуда я вижу» и «докуда видно меня» — после M1 второй наконец имеет основание (радар слышен на 1100 м при своей дальности 420), и показывать только первый значит показывать половину сделки.
- **Тултип в проекте не виджет** — он живёт приватной деталью внутри `ui/context_menu.go`. Поднять в `ui/widget.go` с кадровым слотом отложенной отрисовки (тултип обязан рисоваться поверх всего, как полосы прокрутки).
- **`OrderKindSuppressFire` выключен в UI** с 17.6 и точечного способа его отдать нет. Возвращается вместе со способом назначить точку: ПКМ-удержание по местности (свободных модификаторов нет — `Ctrl` = Sneak, `Alt` = AttackMove, двойной = Sprint).

**Майлстоуны.** L0 одиночник владеет приказом ✅ → L1 охват выделения ✅ → L2 подписи → L3 подавление по точке → L4 гигиена. L0 закрыт: эталон **61/61 IDENTICAL** (сцены зовут `pushSoloMove` напрямую и голову приказа не получают, архетипы не поехали — страховка пережила глубокую правку), новый путь покрыт своей сценой `ai_solo_orders`. Попутно вскрылся латентный баг драйвера: приказ завершается на кольце шире собственного кольца прибытия корпуса, поэтому следующая нога подменяла очередь ДО того, как драйвер сбросит память сторожа «застрял/кружу» — и сторож через несколько секунд вычищал очередь, оставляя корпус за 20 м до цели. L1 закрыт 2026-08-24: `selection_coverage.go` + `ui/selection_coverage.go` — постоянный охват выделения (пунктирные кольца обнаружения по живым каналам, красное кольцо излучения по самому громкому в группе, кольца оружия, приглушённый веер LOS из собственной позиции; борт — только кольца, веер с высоты — диск без информации). Render-only: эталон не сдвинулся.

**Соседство.** Воздушная половина стоит в Phase 20 M5; здесь не дублируется — L0/L1 делают общий механизм для любого одиночника и любого сенсора, M5 добавляет воздушную специфику (кольца ПВО только по ИЗВЕСТНЫМ излучателям).
