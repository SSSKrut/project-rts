...
### Phase 20.7 — Прозрачность командной поверхности 🚧 L0 закрыт (план — `PHASE-20.7-LEGIBILITY.md`)
...
**Майлстоуны.** L0 одиночник владеет приказом ✅ → L1 охват выделения → L2 подписи → L3 подавление по точке → L4 гигиена. L0 закрыт: эталон **61/61 IDENTICAL** (сцены зовут `pushSoloMove` напрямую и голову приказа не получают, архетипы не поехали — страховка пережила глубокую правку), новый путь покрыт своей сценой `ai_solo_orders`. Попутно вскрылся латентный баг драйвера: приказ завершается на кольце шире собственного кольца прибытия корпуса, поэтому следующая нога подменяла очередь ДО того, как драйвер сбросит память сторожа «застрял/кружу» — и сторож через несколько секунд вычищал очередь, оставляя корпус за 20 м до цели.
