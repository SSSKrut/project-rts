...
### Phase 19.8 — Движение 2.1: интерьеры, слоты, уступание дороги 📋 (план 2026-08-09)
...
- M1 📋: нижний слой — стеночный клип для всех позиционных мутаций, Speed=0 в тик остановки, watchdog мёртвой зоны у цели за стеной. Сцена ai_crowd_wall.
- M2 📋: назначение слотов — stateless nearest-match + гистерезис (`LocalBlackboard.HeldSlot`), стабильность к KIA/override; окна без фасинга — round-robin по фасадам. Сцены ai_garrison_house / ai_garrison_kia.
- M3 📋: валидация слот-геометрии — клэмп в комнату (отступ 0.6), запрет лестниц и дверных устий.
- M4 📋: make-way — стоящий уступает под давлением ≥0.3 с или на гейте чужого пути; микрошаг 1 м, возврат через formation re-push. Сцены ai_door_block / ai_stair_block.
- M5 📋: interior-приказ движется как эвакуация (без throttle/prone внутри футпринта). Сцена ai_garrison_underfire.

Не в фазе: очередь входа для Garrison (смотрим после M2+M4), slope-churn марша, интерьерные позиции SurvivalInstinct, LOS сквозь перекрытия. План — `PHASE-19.8.md`.
