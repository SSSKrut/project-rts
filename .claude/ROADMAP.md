...
### Phase 20.7 — Прозрачность командной поверхности 📋 (план — `PHASE-20.7-LEGIBILITY.md`)
...
- Решение фазы — намерение одиночника **на уровне отображения**, не настоящий `Order`. Глубокий вариант («одиночник = отряд из одного») элегантен и даёт историю, приказы и прогресс бесплатно, но заставит `Formation`/`SquadBrain`/`MacroPath` крутиться на каждом грузовике и сдвинет поведение каждой сцены с техникой — отделить намеренное от случайного в таком сдвиге будет нечем. Цена мелкого варианта названа прямо: **у одиночника не будет истории приказов**.
...
**Майлстоуны.** L0 намерение одиночника видно → L1 охват выделения → L2 подписи → L3 подавление по точке → L4 гигиена. L0 не касается симуляции: эталон обязан остаться IDENTICAL, и это и есть проверка, что он сделан правильно.
