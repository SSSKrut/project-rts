...
> **Пивот 2026-08-25 — действующий канон переехал в `.claude/lite/`.**
> Архитектурные принципы этого документа в силе и никуда не денутся (координаты,
> здания в Layer 0, Smart Objects, отряд как сущность, иерархия ИИ). ОБЪЁМ игры —
> нет: таксономия приказов, многоотрядная координация, инженерка и пер-оружейное
> управление вычеркнуты. Список заморозки — `lite/GAME.md`, что строим — там же.
