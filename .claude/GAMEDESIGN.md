...
**Aviation** (sub-phase Phase 16 Vehicles). Воздушные юниты — отдельный locomotion class с 3D-pathfinding, altitude bands (helo 50-200 м, jet 1-5 км), high-speed adaptive movement (не врезаться при 300 m/s). Vision и audio sig сильно отличаются от ground. Подробное проектирование — отдельная сессия перед Phase 16 sub-phase. Архитектурно: Movement system **не должна** предполагать «entity всегда на ground» — Y-координата всегда free, GroundStick применяется только к entity с маркером `OnGround`.
