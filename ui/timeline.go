...
func DrawTimelinePanel(panel Panel, font rl.Font, data TimelineData, view *TimelineViewState,
	...
	rl.BeginScissorMode(int32(gutter.X), int32(gutter.Y+timelineHeaderH),
		int32(gutter.Width), int32(gutter.Height-timelineHeaderH))
	for i, row := range data.Rows {
		drawTimelineRowLabel(font, content, labelW, i, row)
	}
	rl.EndScissorMode()
...
func drawTimelineRowLabel(font rl.Font, content rl.Rectangle, labelW float32,
	rowIdx int, row TimelineSquadRow) {
	rowY := content.Y + timelineHeaderH + float32(rowIdx)*timelineRowH
	if rowY > content.Y+content.Height {
		return
	}
