// Package buildings produces *components.BuildingPlan deterministically from
// a (template, seed, params) triple. Output shape matches the .glb loader so
// downstream systems consume both interchangeably.
//
// Per-building constraint: footprint must lie inside one chunk so all
// children share a single Pos.Chunk. Multi-chunk buildings are out of scope.
package buildings
...
// LevelTransitionSpec.ViaWall). All Local-coord inputs are chunk-local of
// Pos.Chunk. No validation here — run Validate(plan) after Plan().
type Builder struct {
...
// AddWall appends a wall and returns its index. `from`/`to` are chunk-local
// endpoints; length and yaw are derived. `outward` is the OUTWARD-pointing XZ
// unit vector — caller knows which side is outside / cover-providing.
//
// Yaw convention matches WallSegment.Yaw / Stairs.Yaw: after Rotatef(yaw, +Y)
// local +Z points along (from -> to). So yaw=0 along world +Z, pi/2 along +X.
...
// AddStraightStair appends a stair with a 3-waypoint chain (bottom -> middle
// -> top) anchored at endpoints to the two given levels. Use AddCustomStair
// for switchback / spiral geometry. yaw=0 means stair extends along +Z.
...
// two flights of StairsLength/2 meet at a mid landing; total Rise =
// FloorHeight. Footprint = StairsLength x (2*StairsWidth) at `local`
...
// returns along -Z.
//
// Used for Stories >= 3: stacking straight stairs along one wall would
// either run off the floor plate or block the same corner each storey.
func (b *Builder) AddCascadeStair(local rl.Vector3, yaw float32, fromLevelRef, toLevelRef uint8) (int, int) {
	...
	// Local-space (pre-yaw) waypoints, rotated around `local`.
	...
	// Landing at far end of lower flight; upper flight returns along the
	// parallel track (Z back to 0).
...
// AddCustomStair appends a stair with explicit waypoint chain and level
// anchors. For switchback / horizontal-passage topology.
...
// returned by AddWall; pass -1 for stair / passage transitions (anchors
// carry the binding instead).
...
// Plan returns the assembled plan. The Builder may be discarded after this
// call — the returned plan owns its slices.
