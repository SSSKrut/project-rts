...
const (
	...
	gridChan = 6   // cover / allies / enemies / bullets / bullet vx / bullet vz
	...
	hiddenN = 16
	outN    = 3 + neighbours // moveX, moveZ, fire, one preference per enemy slot
...
func (w *World) observeNeighbours(c *Character, side float32, out []float32) {
	...
	writeSlots := func(slots []int, dst []float32) {
		for s, idx := range slots {
			...
			if !w.Phys.Blocked(muzzleOf(c.Position), muzzleOf(o.Position)) {
				f[4] = 1
			}
		}
	}
...
func (w *World) brainIntent(c *Character) Intent {
	...
	for s, idx := range w.slotEnemy {
		...
		if best < 0 || w.act[3+s] > w.act[3+best] {
			best = s
		}
	}
	if best >= 0 && w.act[2] > 0 {
